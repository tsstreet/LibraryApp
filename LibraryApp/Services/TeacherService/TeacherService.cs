...
namespace LibraryApp.Services.TeacherService
{
    public class TeacherService : ITeacherService
    {
        ...
        public async Task<Teacher> UpdateTeacher(int id, Teacher teacher)
        {
            ...
            if (!string.IsNullOrEmpty(teacher.Password) && !string.IsNullOrEmpty(teacher.Name) && !string.IsNullOrEmpty(teacher.Email))
            {
                // Generate password hash and salt
                CreatePasswordHash(teacher.Password, out byte[] passwordHash, out byte[] passwordSalt);
        ...
        public async Task<ICollection<Subject>> GetSubjectByTeacher(int id)
        {
            var subject = await _context.Teachers.Where(x => x.TeacherId == id).Select(c => c.Subjects).FirstOrDefaultAsync();
        ...
        public async Task<List<Teacher>> Search(string searchString)
        {
            ...
            {
                teacher = teacher.Where(s => s.Name.ToLower().Contains(searchString.ToLower())
